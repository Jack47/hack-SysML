...
如果是 Nd 粒度的 DP，把优化器状态分为 Nd 的等份，每个进程负责对应的一部分。所以每个进程只需要存储和更新自己负责的这 1/Nd，只更新了 1/Nd 的参数。在每个 iteration 之后，通过 all-gather 来得到最终所有进程都完全更新后的参数。
...
10. Pos 下，每个人更新自己负责的部分，那也得先把梯度都要过来
