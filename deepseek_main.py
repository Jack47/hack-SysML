...
class DeepseekV2MoE(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        for i in range(self.ep_size):
            start_idx = i * self.experts_per_rank
            end_idx = (i + 1) * self.experts_per_rank
            device_loads_f[i] = f_j[start_idx:end_idx].mean()  # 公式2
            device_loads_P[i] = P_j[start_idx:end_idx].sum()  # 公式3

        device_balance_loss = (device_loads_f * device_loads_P).sum()  # 公式1

        comm_matrix_f = torch.zeros(self.ep_size, device=hidden_states.device)
        comm_matrix_P = torch.zeros(self.ep_size, device=hidden_states.device)
        for i in range(self.ep_size):
            for j in range(self.ep_size):
                if i != j:
                    comm_matrix_f[i] += ((flat_topk_idx >= j * self.experts_per_rank) & (flat_topk_idx < (
                                j + 1) * self.experts_per_rank)).sum().float()  # 计算j设备发给i设备的token，问题在于是不是全发过去???
                    comm_matrix_P[i] += P_j[j * self.experts_per_rank:(j + 1) * self.experts_per_rank].sum()

        comm_balance_loss = (comm_matrix_f * comm_matrix_P).sum()  # 公式1

        total_loss = aux_loss + device_balance_loss + comm_balance_loss
        ...
        if self.training:
            ...
            y = y.view(*orig_shape)
            y = AddAuxiliaryLoss.apply(y, total_loss)
        else:
